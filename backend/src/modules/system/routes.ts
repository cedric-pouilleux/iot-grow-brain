      ...
      try {
        ...
        const response: StorageResponse = {
          ...
          measurements: {
            ...
            oldestRecord:
              rowCountQuery.rows[0].oldest_record instanceof Date
                ? rowCountQuery.rows[0].oldest_record
                : rowCountQuery.rows[0].oldest_record
                  ? new Date(rowCountQuery.rows[0].oldest_record)
                  : null,
            newestRecord:
              rowCountQuery.rows[0].newest_record instanceof Date
                ? rowCountQuery.rows[0].newest_record
                : rowCountQuery.rows[0].newest_record
                  ? new Date(rowCountQuery.rows[0].newest_record)
                  : null,
          },
