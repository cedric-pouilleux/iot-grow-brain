...
export class MqttMessageHandler {
     ...
     */
    private handleSensorStatusMessage(
        topic: string,
        payload: string,
        moduleId: string
    ): boolean {
     ...
     */
    private handleSensorConfigMessage(
        topic: string,
        payload: string,
        moduleId: string
    ): boolean {
    ...
    private handleSensorMeasurement(
        ...
        if (category === 'sensors' && sensorType) {
            ...
            this.fastify.log.info(`📊 Measurement buffered: ${moduleId}/sensors/${sensorType} = ${value} (buffer: ${this.measurementBuffer.length})`);
        ...
        if (parts.length === 2) {
            ...
            this.measurementBuffer.push({ time: now, moduleId, sensorType, value });
            this.fastify.log.info(`📊 Measurement buffered: ${moduleId}/${sensorType} = ${value} (buffer: ${this.measurementBuffer.length})`);
    ...
    private prepareWebSocketData(
        ...
        else if (parsed && (parsed.parts.length === 2 || (parsed.category === 'sensors' && parsed.parts.length === 3))) {
