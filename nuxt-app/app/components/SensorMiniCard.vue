...
onMounted(() => {
  ...
  if (props.history && props.history.length > 0) {
    ...
    initialLastTime.value = last.time instanceof Date ? last.time.getTime() : new Date(last.time).getTime()
  }
})
...
const timeAgo = useTimeAgo(() => {
  ...
  const lastTime = lastItem.time instanceof Date ? lastItem.time.getTime() : new Date(lastItem.time).getTime()
